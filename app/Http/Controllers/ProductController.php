...
class ProductController extends Controller
{
  ...
  public function showEditProduct($id){
      ...
      $data['check'] = DB::select('select * from products where id  = ? ',[$id]);
      if ($data['check'][0]->image == null){
            $data['product'] = DB::select('select *,products.id as productId,products.name as productName,products.image as image from products inner join markets on markets.id = products.market and products.id = ? ',[$id]);
      }else{
  ...
  public function updateProductId(Request $request){
    ...
    if ($request->imageNull != null){
        $media  = new media();
        if ($request->productPriority == 1 ){
            $media->url = $request->chosenImageProduct;
            $media->save();
            $product->image =  $media->id;
        }elseif ($request->productPriority == 0){
            $file=$request->file('productURL');
              if ($file){
                  $ext=$file->getClientOriginalExtension();
                  $fileName=time().'.'.$ext;
        ...
        }elseif($request->productPriority == 3){
            $product->image =  null;
        }else{
            $product->image = null;
        }
        ...
        if ($request->productPriority == 0){
              ...
              $product->image = $media->id;
        }elseif($request->productPriority == 1){

            $media->url = $request->chosenImageProduct;
            $product->image = $media->id;
        }elseif($request->productPriority == 3){
            media::destroy($request->mediaId);
            $product->image = null;
        }else{
            $product->image =  $media->id;
        }
